...
let babies = document.querySelectorAll("[class^='b']:not(.botm)");
...
let turtles = document.querySelectorAll(".tur1");
let turtleData = [];
...
function setFlickyAtGate() {
  ...
  x = gateRect.right - containerRect.left + 20;
...
function initializeTurtles() {
  turtleData = Array.from(turtles).map((tur, index) => {
    const container = tur.parentElement;

    const minX = 0;
    const maxX = container.clientWidth - tur.offsetWidth;
    ...
    if (index === 0) {
      ...
      startX = container.clientWidth - tur.offsetWidth; 
      direction = -1;
    }

    tur.style.left = startX + "px";
    tur.style.top = (container.clientHeight - tur.offsetHeight) + "px";
...
turtleData.forEach((turObj) => {
  turObj.x += turObj.speed * turObj.direction;

  if (turObj.x <= turObj.minX || turObj.x >= turObj.maxX) {
    turObj.direction *= -1;
    turObj.x = Math.max(turObj.minX, Math.min(turObj.x, turObj.maxX));
  }

  turObj.el.style.left = turObj.x + "px";

  if (!catHitCooldown && isColliding(flicky, turObj.el)) {
    loseLife();
  }
});
